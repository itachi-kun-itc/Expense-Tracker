(() => {
  ...
  function renderAttendance() {
    ...
    attendanceView.innerHTML=`<div class="academic-heading section-heading"><div><p class="eyebrow">ATTENDANCE</p><h2>出欠管理</h2><p>遅刻3回を欠席1回相当として目安を計算</p></div><button class="primary-btn" data-add-subject>＋ 科目</button></div><div class="attendance-summary"><small>平均出席率</small><b>${average.toFixed(1)}%</b></div><div class="subject-list">${academic.subjects.length?academic.subjects.map(subject=>`<article class="panel subject-row" data-subject-id="${subject.id}"><label>科目名<input data-subject-field="name" value="${escapeHtml(subject.name)}" maxlength="30" placeholder="例：英語"></label><label>実施回数<input type="number" min="0" data-subject-field="held" value="${subject.held}" placeholder="例：15"></label><label>出席<input type="number" min="0" data-subject-field="attended" value="${subject.attended}" placeholder="例：14"></label><label>遅刻<input type="number" min="0" data-subject-field="late" value="${subject.late}" placeholder="例：1"></label><label>欠席<input type="number" min="0" data-subject-field="absent" value="${subject.absent}" placeholder="例：0"></label><div class="subject-rate"><span>出席率</span><b>${attendanceRate(subject).toFixed(1)}%</b><i><em style="width:${attendanceRate(subject)}%"></em></i></div><button data-delete-subject aria-label="科目を削除">×</button></article>`).join(""):'<div class="panel salary-empty">科目を追加してください</div>'}</div>`;
  }
  ...
  function renderCommuter() {
    ...
    commuterView.innerHTML=`<div class="academic-heading"><p class="eyebrow">COMMUTER PASS</p><h2>定期券計算</h2><p>通常運賃と通学定期券を比較</p></div><div class="commuter-layout"><article class="panel commuter-form"><div class="form-grid"><label>片道運賃（円）<input type="number" min="0" data-commuter-field="oneWay" value="${value.oneWay}" placeholder="例：220"></label><label>月の通学日数<input type="number" min="0" max="31" data-commuter-field="daysPerMonth" value="${value.daysPerMonth}" placeholder="例：20"></label></div><label>比較期間<select data-commuter-field="months"><option value="" ${value.months===""?"selected":""} disabled>例：3か月</option>${[1,3,6,12].map(number=>`<option value="${number}" ${Number(value.months)===number?"selected":""}>${number}か月</option>`).join("")}</select></label><div class="pass-price-grid"><label>1か月定期<input type="number" min="0" data-commuter-field="pass1" value="${value.pass1}" placeholder="例：8,000"></label><label>3か月定期<input type="number" min="0" data-commuter-field="pass3" value="${value.pass3}" placeholder="例：22,800"></label><label>6か月定期<input type="number" min="0" data-commuter-field="pass6" value="${value.pass6}" placeholder="例：43,200"></label></div></article><article class="panel commuter-result"><div><small>通常運賃</small><b>${money(normal)}</b></div><div><small>定期券</small><b>${pass?money(pass):"未入力"}</b></div><div class="saving-result"><small>${saving>=0?"定期券がお得":"通常運賃がお得"}</small><strong>${money(Math.abs(saving))}</strong></div></article></div>`;
  }
