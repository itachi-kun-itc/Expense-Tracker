...
function renderExpenseBreakdown() {
  ...
  expenseBreakdown.innerHTML=`<div class="expense-total"><small>収入－支出</small><b>${yen(monthTotal.balance)}</b></div>${values.length?`<div class="expense-ratio-list">${values.map(item=>{const category=cats[item.key],percent=Math.round(item.value/total*100);return `<div><span>${category.name}</span><div class="ratio-bar"><i style="width:${percent}%;background:${category.color}"></i></div><b>${percent}%</b><small>${yen(item.value)}</small></div>`}).join("")}</div>`:'<p class="salary-empty">この月の支出はありません</p>'}`;
}
