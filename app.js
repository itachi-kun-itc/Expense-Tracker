...
const cardMemoDialog=document.createElement("dialog");cardMemoDialog.id="cardMemoDialog";cardMemoDialog.innerHTML=`<form method="dialog" id="cardMemoForm"><div class="modal-head"><div><p class="eyebrow">CARD MEMO</p><h2>カード履歴を追加</h2></div><button type="button" class="close-btn" data-card-close>×</button></div><div class="form-grid"><label>利用日<input id="cardDateInput" name="date" type="date" required></label><label>金額<input name="amount" type="number" min="1" required placeholder="3,000"></label></div><label>カード名<input name="cardName" type="text" maxlength="30" placeholder="メインカード"></label><label>メモ<input name="memo" type="text" maxlength="60" placeholder="動画配信サービス"></label><label class="card-recurring-option"><input type="checkbox" name="recurring"><span><b>毎月自動的に記録する</b><small>サブスクなど、毎月同じ日に記録します</small></span></label><button class="primary-btn wide">保存する</button></form>`;document.body.appendChild(cardMemoDialog);const cardMemoForm=cardMemoDialog.querySelector("form"),cardDateInput=cardMemoDialog.querySelector("#cardDateInput");
